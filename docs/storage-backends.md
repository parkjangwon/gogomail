...
concatenated diagnostics. Same-local-name safe preview fields from foreign XML
namespaces are also suppressed rather than interpreted as canonical S3 error
text.
...
singular, nonblank, and a bounded printable ASCII opaque value; providers that
omit the optional header remain compatible.
...
when both raw header metadata and Go's normalized response length are known,
they must agree even when the normalized response length is zero. When the
response length is known, the returned reader is bounded to that size and
reports `io.ErrUnexpectedEOF` if the provider truncates the body.
...
namespace-free or AWS S3-namespaced, use known AWS child names, do not carry
direct text on the structured wrapper element, and do not repeat the same child
name. Duplicate single-value `StorageClass`,
...
and present ETags must remain nonblank, unpadded, non-double-quoted, printable
ASCII opaque values after the same bounded metadata cleanup as `Stat` instead
of being silently dropped. Present
