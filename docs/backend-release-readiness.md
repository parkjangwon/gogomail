...
- Domain DNS check, quota reconciliation correction, trusted relay create/delete, delivery route create/status/delete, and DKIM key lifecycle audit writes reuse the shared hash-chain writer, so their newly visible audit rows carry non-empty tamper-evidence hashes while excluding relay and private-key secrets from audit detail.
