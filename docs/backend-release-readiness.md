...
- Message detail read-path caching can be tuned with `GOGOMAIL_MESSAGE_BODY_CACHE_ENTRIES` (default `256`, `0` disables) and `GOGOMAIL_MESSAGE_BODY_CACHE_TTL` (default `5m`), with service-level cache snapshots exposing hits, misses, entries, capacity, TTL, and evictions for future metrics adapters.
