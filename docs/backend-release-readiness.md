...
- API metering has a disabled-by-default aggregation worker and daily/monthly Postgres read models exposed through `GET /admin/v1/api-usage/daily` and `GET /admin/v1/api-usage/monthly`; v2 events carry tenant/company/domain/user/API-key/principal/auth-source dimensions plus deterministic IDs, request identity dimensions are whitespace-normalized, auth-source values are normalized to a fixed known set, blank bearer headers are not classified as bearer traffic, route keys and HTTP-like status values are required, CR/LF-bearing method/route/event-id/identity dimensions are rejected before outbox insertion and ledger storage, aggregate store direct calls also validate event IDs, schema versions, identity dimensions, and HTTP-like status before writes, middleware falls back to `METHOD /path` when no ServeMux pattern is available, negative byte/latency metrics are clamped to zero before outbox payload generation and aggregate storage, replayed event IDs are not double-counted, and aggregates are keyed by identity. The worker also records immutable `api_usage_ledger` rows with bounded Admin API list, NDJSON export, stats, non-future-cutoff retention readiness, trimmed tenant/principal query filters, persisted export batch manifests/checkpoints that require explicit `from`/`to` windows, local object-store artifact writing/download/verification, external artifact metadata handoff, canonical manifest digest verification, disabled-by-default local-HMAC/local-Ed25519/remote-Ed25519 manifest signing and verification behind signer/verifier interfaces, export capability inspection, and a handoff readiness report that separates operational readiness from billing readiness. The handoff report can opt into `deep=true` to stream artifacts, check latest digest artifact coverage, and verify latest digest/signature evidence in one operator response with separate `verified_billing_ready`. Invoice/hard-limit use should rely on `remote-ed25519` only when backed by an approved KMS signing service, or on a future direct cloud KMS adapter.
...
- Mail compose draft/save/send requests normalize user/source/from/address and
  ...
  outbound composition work, reject CR/LF-bearing recipient display
  names/emails before draft persistence or header composition, and draft saves
  enforce the same attachment-count cap as immediate sends.
