...
- Domain DNS check, quota reconciliation correction, trusted relay create/delete, and delivery route create/status/delete audit writes reuse the shared hash-chain writer, so their newly visible audit rows carry non-empty tamper-evidence hashes.
