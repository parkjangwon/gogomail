...
- API metering has a disabled-by-default aggregation worker and daily/monthly Postgres read models exposed through `GET /admin/v1/api-usage/daily` and `GET /admin/v1/api-usage/monthly`; v2 events carry tenant/company/domain/user/API-key/principal/auth-source dimensions plus deterministic IDs, replayed event IDs are not double-counted, and aggregates are keyed by identity. The worker also records immutable `api_usage_ledger` rows with bounded Admin API list, NDJSON export, stats, persisted export batch manifests/checkpoints, local object-store artifact writing/download, external artifact metadata handoff, and canonical manifest digest verification, but invoice/hard-limit use still needs signing/KMS integration and paginated/partitioned export writing.
