...
- Attachment upload size is guarded in HTTP and service layers.
...
- Compose and draft validation guard user id, intent/source rules, recipient presence, recipient email blanks, recipient count, subject size, text body size, attachment IDs, filename safety, MIME type, and upload size.
- API errors use a stable structured envelope with code, message, and HTTP status.
- Admin API supports domain/user list, create, and status updates plus queue, delivery-attempt, suppression, DKIM, retry, and delete operations.
