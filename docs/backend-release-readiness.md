...
- CalDAV and CardDAV collection `DELETE` now honors `If-None-Match`
...
- CalDAV and CardDAV collection `PROPPATCH` now honors the same
  ...
  semantics.
- CalDAV and CardDAV collection `DELETE` and `PROPPATCH` now pass observed
...
- IMAP search-family numeric operands now also reject exact quoted or
  literal-framed values for `LARGER`, `SMALLER`, and MODSEQ thresholds/entry
  types, while preserving quoted MODSEQ entry names as RFC string operands.
...
- IMAP `KEYWORD` and `UNKEYWORD` search operands now reject quoted
  ...
  RFC atom boundary.
- IMAP `STORE` and `UID STORE` mutation controls now reject quoted or
  command-literal flag update mode tokens and quoted `UNCHANGEDSINCE` markers
  before authentication or selected-mailbox mutation state.
...
- IMAP `SELECT` now canonicalizes backend-provided permanent flags before
  ...
  permission checks, keeping response metadata duplicate-free, RFC-shaped, and
  aligned with the permitted mutation set.
...
- S3-compatible `ListObjectsV2` object `ETag` metadata now rejects
  present-but-blank, whitespace-padded, malformed, line-bearing,
  double-quoted, empty-after-quote cleanup, non-ASCII, non-printable, or
  oversized values instead of silently dropping suspect listed-object identity
  metadata. The same printable ASCII ETag boundary is shared by optional
  `PutObject` success headers, `HEAD`/`Stat`, and `CopyObjectResult` XML.
...
- API usage export batch list/create/detail/export OpenAPI operations now also
  pin the Admin API server and admin-token/bearer auth alternatives, matching
  their runtime `adminAuth` routes.
- API usage export artifact list/create/detail/write/download/verification
  OpenAPI operations now also pin the Admin API server and admin-token/bearer
  auth alternatives, matching their sensitive runtime `adminAuth` routes.
...
- Tenant, domain, and user administration OpenAPI operations now also pin the
...
- Mail API list/search handlers apply the documented default `limit=50` when
  `limit` is omitted or empty, with regression coverage for message lists,
...
- IMAP `LIST` and `LSUB` normalize leading hierarchy delimiters in mailbox
...
- IMAP UIDPLUS `APPENDUID` response codes are suppressed when append results
  mark returned UID metadata as non-sticky, extending the same non-persistent
  UID boundary to APPEND.
...
- IMAP repeated `ENABLE CONDSTORE` is now covered after `SELECT ... (CONDSTORE)`
...
- CalDAV has an internal `OPTIONS`/`PROPFIND` discovery handler boundary with
  ...
  authenticated user/path scoping, no-store/nosniff discovery and
  unsupported-method headers, and multistatus rendering over a pluggable
...
- CalDAV sync-change retention pruning now has repository groundwork:
  ...
  CalDAV/CardDAV candidate/deleted counts, including partial failures. The DAV
  ...
  plus CalDAV/CardDAV candidate counts with truncation/readiness flags. Admin
  API can now execute dry-run or explicitly confirmed destructive DAV sync
  retention runs through `/admin/v1/dav-sync/retention-runs`; destructive runs
  reuse the readiness preview and fail closed when either backend hits the
  probe limit. Optional PostgreSQL integration coverage now applies release
  ...
  still requires documented token-retention policy and native-client expiry
  testing.
...
- CardDAV is pre-public and backend-only. ADR 0012 and `internal/carddavgw`
  ...
  CalDAV `OPTIONS` and unsupported-method `Allow` headers now have explicit
  regression coverage that future WebDAV `COPY`/`MOVE` constants remain
  unadvertised until calendar-object relocation and duplication semantics
  exist.
  ...
  `Depth` header, uses `Depth: 1` for child address-object scans, keeps
  `Depth: 0` collection-scoped without returning child objects, and rejects
  `Depth: infinity` before XML body parsing. PROPFIND
  ...
  XML request bodies. Repeated date conditionals now fail closed across
  contact-object reads/writes/deletes and collection precondition checks, so
  CardDAV timestamp guards cannot depend on first-header behavior in clients
  or intermediaries. RFC 6352-style extended `MKCOL` can create authenticated
...
- IMAP `FETCH`/`UID FETCH` regression coverage includes partial-window empty
  top-level header-field-list forms, so preview probes such as
  `BODY.PEEK[HEADER.FIELDS ()]<0.1>` remain deterministic.
...
- IMAP `SEARCH HEADER` validates header field names at the grammar boundary,
  ...
  `HEADER.FIELDS` filtering matches raw RFC 5322 field names without trimming
  malformed whitespace before the colon.
...
- IMAP BODY/BODYSTRUCTURE MIME metadata now validates media type, subtype,
...
- IMAP command parsing rejects control characters or 8-bit non-ASCII bytes
  inside unquoted atoms, quoted strings, parenthesized quoted controls, and
  RFC 2971 ID quoted tokens, matching RFC 3501's 7-bit atom/string boundary
...
- IMAP `APPEND` now has a protocol-to-backend request boundary for mailbox,
  ...
  `"5-May-2026 ..."`, and canonicalizes date-month atoms ASCII-case-insensitively
...
- `scripts/verify-backend-release.sh` runs the standard backend release checks (`go test ./...`, `go mod tidy -diff`, optional PostgreSQL integration tests when `GOGOMAIL_TEST_DATABASE_URL` is set, optional backup/restore rehearsal when `GOGOMAIL_RESTORE_REHEARSAL_DATABASE_URL` is set, optional security verification with `go vet` and `govulncheck` when `GOGOMAIL_SECURITY_VERIFY=1`, optional OpenSearch integration coverage when `GOGOMAIL_TEST_OPENSEARCH_URL` is set, and a clean-worktree gate that fails on pending repository changes). The rehearsal wrapper passes `GOGOMAIL_RESTORE_REHEARSAL_DATABASE_URL` as `GOGOMAIL_DATABASE_URL`; `scripts/backup-restore-rehearsal.sh` can further override the scratch target with `GOGOMAIL_RESTORE_REHEARSAL_DB_URL` and keep it for inspection with `GOGOMAIL_RESTORE_REHEARSAL_KEEP_DB=1`. If `govulncheck` is not installed, the script falls back to `go run golang.org/x/vuln/cmd/govulncheck@latest`.
- Message detail read-path caching can be tuned with `GOGOMAIL_MESSAGE_BODY_CACHE_ENTRIES` (default `256`, `0` disables) and `GOGOMAIL_MESSAGE_BODY_CACHE_TTL` (default `5m`), with service-level cache snapshots exposing hits, misses, entries, capacity, TTL, evictions, and expired-entry removals for future metrics adapters.
