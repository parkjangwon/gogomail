...
both globally (Do-Not-Disturb) and on a per-folder basis.
...
notifications enabled for every folder, no DND, UTC timezone, empty
`folder_overrides`.
...
### Per-folder vs global precedence

For a given folder, notifications fire iff **all** of the following hold:

1. `folder_overrides[folder_id].enabled` is `true` (default `true` when no
   override is present).
2. The folder is **not** currently in DND.
...
Thus `folder.enabled = false` silences a folder unconditionally regardless of
DND windows, and `dnd_inherit` lets the user pick between the global schedule
and a local one.
...
Persisted in the `notification_preferences` table (migration 0149). The row is
cascaded on user delete. `updated_at` is set by the database on every upsert
and surfaced in the API response for client-cache invalidation.
