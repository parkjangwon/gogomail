...
POP3 기본 `USER/PASS` 인증이 실패하면 서버는 authorization 상태를 유지해야 한다.
가장 흔한 인증 실패 경로가 capability나 후속 로그인 흐름을 오염시키지 않는지
고정한다.
...
- [x] `USER/PASS` wrong password가 `-ERR authentication failed`를 반환하는지 검증한다.
