...
- [x] Multiple `RCPT TO` commands에서 recipient-specific DSN 옵션(NOTIFY, ORCPT)이 정확하게 추적되는지 검증한다.
- [x] 한 RCPT에 설정된 DSN 옵션이 다음 RCPT에 누출되지 않는지 검증한다.
- [x] NOTIFY 파라미터 다양성 테스트: NEVER, SUCCESS, FAILURE, DELAY 조합을 검증한다.
- [x] ORCPT (Original Recipient) 파라미터가 올바르게 인코딩되고 보존되는지 검증한다.
- [x] 같은 recipient를 여러 번 RCPT할 때 마지막 DSN 옵션이 적용되는지 검증한다.
- [x] `go test -count=1 ./internal/smtp -run TestSubmissionRcptDSN` 통과.
- [x] `go test ./...` 통과.
