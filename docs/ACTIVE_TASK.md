...
- SMTP delivery pipelining now precomputes DSN RCPT option parameters by normalized address once per batch, avoiding repeated linear DSN recipient scans on large DSN-enabled sends.
- Redis-backed SMTP farm healthy-node discovery now uses SCAN plus pipelined TTL/hash reads instead of blocking KEYS, reducing production Redis keyspace risk.
...
- Password-reset request handling now decouples accepted background token/email work from the client request context with a 30-second bounded context.
...
- outbox relay claim 후보 조회에 `0120_outbox_claim_indexes.sql` partial indexes를 추가해 due pending row와 stale processing lock row를 각각 availability/lock-age 기준으로 찾도록 보강함
...
- production config가 `GOGOMAIL_SMTP_DOMAIN`과 `GOGOMAIL_DELIVERY_SMTP_HELLO`의 localhost/loopback/unspecified 값을 거부하도록 막아 SMTP banner, Received/DSN/TLS-RPT identity, outbound EHLO가 개발용 hostname으로 출시되지 않게 함
...
- 웹메일 보안 설정의 하드코딩된 가짜 세션 목록을 제거하고, 실제로 보장 가능한 현재 세션/전체 로그아웃 상태만 표시하며 revoke 실패는 브라우저 alert 대신 inline error로 표시하도록 정리함
...
- 관리자 domain 목록 조회도 company/status/DNS status 필터가 있을 때만 WHERE에 추가하도록 바꿔 도메인 운영 목록의 optional `OR` predicate를 제거함
...
- 관리자 사용자 목록 조회도 domain/status/password_configured 필터를 제공된 경우에만 WHERE에 추가하고, domain 필터는 typed UUID predicate로 바꿔 사용자 운영 목록의 optional `OR` predicate를 제거함
- 관리자 quota usage 목록 조회도 scope/domain/over_limit/over_allocated 필터를 제공된 경우에만 WHERE에 추가해 quota 운영 화면의 optional `OR` predicate를 제거함
- 관리자 사용자 bulk status 업데이트도 company scope가 있을 때만 `d.company_id = $3::uuid` predicate를 추가해 company_admin 제한 경로의 optional `OR` predicate를 제거함
- API usage ledger retention readiness의 covering export batch lookup은 `window_start IS NULL OR ...` 대신 `COALESCE(window_start, '-infinity')` expression predicate를 사용하고, `0118_api_usage_export_covering_batch_index.sql`로 tenant/principal/window range/order partial index를 추가함
...
- 웹메일 설정 가져오기 실패가 브라우저 alert로 흐름을 끊지 않고 settings 화면 안의 inline 상태 메시지로 표시되도록 바꾸고, 설정 export key 목록의 중복 항목을 제거함
- admin 회사 사용자 조회 헬퍼가 domain별 `ListUsers` 반복 호출 대신 `CompanyID` scope를 직접 지원하는 단일 `ListUsers` 쿼리를 사용하도록 바꿔 bulk export/SCIM status/security posture/seat usage/user limit 체크의 N+1 조회를 제거함
- LDAP gateway search 결과의 `member`/`memberOf` 확장이 principal별 `ListGroupMemberships` 반복 호출 대신 group/member ordinality batch query로 한 번씩 조회하도록 바꿔 그룹/사용자가 많은 디렉터리 검색의 DB 왕복을 줄임
