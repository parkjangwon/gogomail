...
- `GET /api/v1/drive/share-links/{token}/download` and
  ...
  missing/revoked/expired links to HTTP 404. Malformed or unsatisfiable shared
  byte ranges return HTTP 416 with `Content-Range: bytes */<size>` after a
  metadata stat and before any full or ranged object open. OpenAPI documents
  the public
  shared-download `HEAD`, full-body `200`, and byte-range `206` responses as
  non-JSON binary/header contracts, plus the shared-download `416`
  `Content-Range` header, so generated Drive clients do not treat shared file
  bytes or range errors as undocumented shapes. The download operations are
  marked unauthenticated to match the runtime public-share boundary.
  ...
  actions `share_link.resolve`, `share_link.download`, and
  `share_link.download_head`. Audit details include node id and permission when
  available plus token suffix, result, status, and requested byte range when
  present, but never raw tokens or storage backend/path values.
...
- `GET /admin/v1/audit-logs` returns `{"audit_logs":[...]}` and supports
  bounded `limit`, `category`, exact `action`, `action_prefix`, `result`,
  `target_type`, `company_id`, `domain_id`, `user_id`, `actor_id`,
  `target_id`, and RFC3339 `since` filters for operational forensics over
  persisted audit records, including delegated access checks by actor or target
  owner principal and action families such as `share_link.`. Text filters
...
- `GET /admin/v1/mail-flow-logs/daily-stats` returns
  ...
  `outbound_size_bytes`, `delivered`, `failed`, `bounced`, `filtered`, and
  `rejected` counts. It supports `direction`, `company_id`, `domain_id`,
  `user_id`, and RFC3339 `since`/`until` filters. Results are ordered by date
  descending and include zero-filled days when both time bounds are supplied.
...
- `GET /admin/v1/api-usage/export-capabilities` returns
  ...
  remote-key requirement for destructive worker retention. OpenAPI pins this
  operation to the Admin API server and documents the same `X-Admin-Token` /
  bearer-token alternatives as the Admin Console capability bootstrap.
