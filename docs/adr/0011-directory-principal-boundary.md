...
principals. The first implementation slice resolves active user principals from
the existing `users`, `domains`, and `companies` tables, with bounded principal
identifier validation and explicit principal kinds.
...
- Active user principal lookup is now shared and testable outside CalDAV.
- Future organization, group, resource, alias, membership, and delegation
