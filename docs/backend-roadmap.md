...
- SearchResultEntry responses are encoded as RFC 4511 application payloads and covered by OpenLDAP `ldapsearch` plaintext, Assertion control, Matched Values control, Domain Scope/Don't Use Copy/Subentries controls, LDAP Sync refreshOnly, Proxied Authorization, extensibleMatch, server-side sort, Virtual List View, StartTLS, LDAPS, and paged-results smoke tests when the client is available.
