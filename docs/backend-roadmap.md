...
- SearchResultEntry responses are encoded as RFC 4511 application payloads and covered by OpenLDAP `ldapsearch` plaintext, Assertion control, Matched Values control, extensibleMatch, server-side sort, Virtual List View, StartTLS, LDAPS, and paged-results smoke tests when the client is available.
...
- LDAPMessage controls are parsed separately from protocolOp bytes; supported critical controls (`ManageDsaIT`, Simple Paged Results, Server Side Sort, Virtual List View, Assertion, Matched Values) are accepted, Simple Paged Results returns continuation cookies, Server Side Sort orders entries and returns the RFC response control, Virtual List View returns sorted position/count windows, Assertion validates RFC 4528 filters before search dispatch, Matched Values applies RFC 3876 filters to response attribute values, and unsupported critical controls return `unavailableCriticalExtension`.
