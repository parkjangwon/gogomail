...
759. The shared MIME-structure parser now descends into `message/rfc822` parts
...
770. IMAP `FETCH` and `UID FETCH` now stream actual multipart child MIME
     headers for `BODY[n.MIME]` and `BODY.PEEK[n.MIME]` requests when the
     selected part exists.
771. IMAP `FETCH` and `UID FETCH` now support bounded nested multipart
     body-section literals such as `BODY[1.2]` with a capped MIME part path
     depth.
772. IMAP `FETCH` and `UID FETCH` now support bounded partial windows over
...
778. IMAP `CREATE`, `DELETE`, and `RENAME` now have protocol and service
...
779. IMAP `\Deleted` is now a first-class protocol flag in the gateway and
...
780. IMAP `EXPUNGE` and `UID EXPUNGE` now have protocol, service, repository,
     ...
     semantics.
781. IMAP `MOVE` and `UID MOVE` now have protocol, service, repository, and
...
782. IMAP now advertises `UIDPLUS` and returns RFC 4315-style `[COPYUID ...]`
     ...
     message identities.
783. IMAP `CLOSE` now follows RFC selected-mailbox semantics by silently
...
784. IMAP `SELECT` and `EXAMINE` now emit optional RFC-shaped `[UNSEEN n]`
...
785. IMAP command reading now consumes bounded synchronizing literals with a
...
786. IMAP `APPEND` now has a protocol-to-backend request boundary carrying the
...
787. IMAP `APPEND` request parsing now accepts RFC-shaped optional flag lists and
...
788. IMAP `APPEND` success responses now have a UIDPLUS-ready result boundary
...
789. `mailservice` now routes IMAP `APPEND` through an append-capable
...
790. IMAP `APPEND` now has a service-backed storage path: the service spools
     ...
     one transaction.
791. IMAP `APPEND` now maps missing destination mailboxes to an RFC-shaped
     `NO [TRYCREATE]` response code, improving compatibility with clients that
     can create the target mailbox and retry the append.
792. IMAP `APPEND` now maps quota-ledger `mailbox full` failures to a
...
793. IMAP `APPEND` now rejects commands without a synchronizing literal as a
     ...
     payload.
794. IMAP `APPEND` persistence now returns the appended message sequence number
...
795. IMAP `COPY`, `UID COPY`, `MOVE`, and `UID MOVE` now map missing
     ...
     clients.
796. IMAP selected-mailbox event draining now suppresses stale or duplicate
     ...
     mailbox size.
797. IMAP mailbox DTOs now carry durable `highest_modseq`, and `SELECT`,
     `EXAMINE`, and `STATUS` can expose `[HIGHESTMODSEQ ...]` /
     `HIGHESTMODSEQ` metadata for clients that use mod-sequence based sync
     hints.
798. IMAP `FETCH` and `UID FETCH` now return RFC 4551-shaped `MODSEQ (n)`
     attributes when requested, mapping durable per-message IMAP mod-sequences
     through the gateway.
799. IMAP `SEARCH` and `UID SEARCH` now support RFC 4551-shaped `MODSEQ`
...
800. IMAP `FETCH` and `UID FETCH` now support RFC 4551-shaped `CHANGEDSINCE`
...
801. IMAP sessions now become CONDSTORE-aware after implemented mod-sequence
...
802. IMAP `STORE` and `UID STORE` now support RFC 4551-shaped
...
803. Conditional IMAP `STORE`/`UID STORE` response and service event paths now
     ...
     backend adapters.
804. IMAP `SELECT` and `EXAMINE` now accept the RFC 4551-shaped `(CONDSTORE)`
     parameter and mark the session CONDSTORE-aware.
805. IMAP `CAPABILITY` now advertises `CONDSTORE`, making the implemented
     RFC 4551 durable mod-sequence sync surface discoverable by standard IMAP
     clients.
806. IMAP now advertises `ENABLE` and accepts RFC 5161-shaped
...
807. IMAP `LIST` now emits RFC 6154 special-use attributes for system folder
...
808. IMAP now advertises `SPECIAL-USE` and accepts RFC 6154 extended
...
809. Development storage portability is now documented in
...
810. S3-compatible storage URL generation now has regression coverage for
...
811. IMAP selected-mailbox event draining now renders sequence-bearing
     `MailboxEventExpunge` notifications as untagged `EXPUNGE` responses for
     `NOOP`/`IDLE` clients, keeping live deletion state aligned.
812. Existing IMAP UID lookup now returns mailbox sequence numbers, letting
     ...
     rows.
813. IMAP `MOVE` and `UID MOVE` now reassign source mailbox UID rows to fresh
...
814. IMAP `MOVE` and `UID MOVE` now advance the selected source mailbox
...
815. IMAP `CAPABILITY` now advertises RFC 3348 `CHILDREN` alongside the
     existing `\HasNoChildren` LIST attributes, keeping mailbox hierarchy
     discovery signals consistent for standards-aware clients.
816. IMAP `LIST` now derives RFC 3348 `\HasChildren` / `\HasNoChildren`
...
817. IMAP `SUBSCRIBE` and `UNSUBSCRIBE` now persist mailbox subscription names
...
818. IMAP `ID` now validates RFC 2971-shaped `NIL` or bounded field/value
     ...
     identity.
819. IMAP `MOVE` and `UID MOVE` now allow the selected mailbox as the
     ...
     RFC 6851.
820. IMAP `CAPABILITY` now advertises RFC 5819 `LIST-STATUS`, and extended
     ...
     round trips.
821. IMAP `CAPABILITY` now advertises RFC 4731 `ESEARCH`; `SEARCH RETURN (...)`
...
822. IMAP `CAPABILITY` now advertises RFC 5182 `SEARCHRES`; `SEARCH RETURN
...
823. IMAP `CAPABILITY` now advertises RFC 8438 `STATUS=SIZE`; `STATUS` and
...
824. IMAP `CAPABILITY` now advertises RFC 5256 `SORT`; `SORT` and `UID SORT`
     ...
     ordering.
825. IMAP `CAPABILITY` now advertises RFC 5256 `THREAD=ORDEREDSUBJECT`;
...
826. IMAP RFC 5256 base-subject extraction now decodes RFC 2047 encoded-word
...
827. IMAP mailbox mutation guardrails now enforce RFC 3501 `INBOX` special
     ...
     implemented.
828. Service-backed IMAP message summaries now hydrate stored `To`, `Cc`, and
...
829. IMAP `FETCH` and `UID FETCH` now apply RFC 3501 `\Seen` side effects for
     successful `BODY[...]`, `RFC822`, and `RFC822.TEXT` literal reads while
     preserving `BODY.PEEK[...]` and `RFC822.HEADER` as non-mutating preview
     requests.
830. IMAP `FETCH` and `UID FETCH` now preserve RFC 3501 `RFC822`,
...
831. IMAP `SUBSCRIBE` can now persist missing mailbox names so `LSUB` can
     report them with `\Noselect`, preserving subscription state across mailbox
     migration, deletion, and delayed creation flows.
832. IMAP mailbox names now follow RFC 3501 modified UTF-7 at the protocol
