...
calendar's configured timezone rather than always UTC.
 ...
 payloads (using direct Value field access for CAL-ADDRESS type properties) and
 builds RFC 6047 compliant iTIP messages with multipart/alternative MIME format.
 Queue and ObjectStore interfaces allow actual SMTP mail sending when wired in;
 currently logs attendee info for RFC 6047 iMIP participant extraction.
