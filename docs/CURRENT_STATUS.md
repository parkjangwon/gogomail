...
- LDAP controls are parsed at the LDAPMessage boundary; supported critical controls (`ManageDsaIT`, Simple Paged Results, Server Side Sort) are accepted, unsupported critical controls return `unavailableCriticalExtension`, Simple Paged Results uses opaque offset cookies across search pages, and Server Side Sort returns the RFC response control after ordering entries.
