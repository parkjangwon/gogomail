...
- Added typed account/context, bulk mail, Drive download/upload-session/share-link, simple vCard contact, and simple ICS event helpers so agents can use GoGoMail services without hand-writing low-level payloads for common workflows.
...
- Titles that normalize to blank fall back to `Notification`, while optional bodies that normalize to blank are dropped before state, persistence, or browser notification mirroring.
- Existing title/body length caps remain in place after normalization.
- This prevents malformed runtime or stored notification payload text from visually splitting notification center rows or native browser notification mirrors.
- E2E coverage verifies runtime and stored title/body control characters are normalized, blank-after-normalization titles fall back, blank bodies are removed, and existing text truncation/malformed-field behavior remains intact.
- Verification target: `pnpm -C apps/webmail exec playwright test e2e/notifications.spec.ts --project=chromium -g "normalizes control characters in notification text|truncates oversized notification text|normalizes malformed runtime notification fields"`; `pnpm -C apps/webmail type-check`.
...
- `GOGOMAIL_PUBLIC_BASE_URL` no longer has a localhost default, so staging/dev-like deployments with unset env avoid accidental localhost tracking pixels too.
...
- Attachment upload-session listing now builds optional user, draft, and status predicates only when requested, keeping upload-operation views clear of broad optional `OR` guards.
...
- Message page read/starred/attachment filters now emit direct predicates only when requested and omit nullable boolean optional `OR` branches for unfiltered lists.
- Message page cursor predicates now emit direct newest/oldest tuple comparisons only when a pagination cursor is present and omit pagination optional `OR` branches for first-page lists.
...
- Admin domain listing now builds optional company/status/DNS status predicates only when requested, keeping tenant domain operations clear of broad optional `OR` guards.
...
- DKIM key listing now builds optional domain and status predicates only when requested, keeping domain-authentication operations clear of broad optional `OR` guards.
...
- Directory principal search now emits only requested principal-kind UNION branches and optional domain, organization, query, and active-status predicates, keeping address-book lookup paths aligned with selective indexes.
- CalDAV/CardDAV sync-change retention pruning now builds scoped predicates only when user/calendar/addressbook filters are provided, avoiding optional `OR` guards in scheduled DAV cleanup scans.
...
- **Component Extraction Progress**:
  ...
  - `apps/webmail/src/components/calendar/MonthView.tsx` (134 lines)
  - CalendarView.tsx: 1887 → 1372 lines (-515 lines, **-27.3% reduction**)
  - Total extracted for calendar: 607 lines in 5 focused modules
- **Refactoring Pattern Established**:
  - Created `docs/FRONTEND_REFACTORING_ROADMAP.md` documenting complete approach
  - Pattern: Extract utilities → Extract sub-components → Organize by feature
  - Roadmap shows 30-40 commits needed for complete webmail+console refactoring
- **Next Steps** (remaining ~1000 lines in CalendarView):
  1. Extract WeekView and DayView components
...
## Mail Flow Logs query UI (2026-05-15, complete)
...
## Audit Logs query UI (2026-05-15, in progress)
- `apps/console/src/app/companies/[id]/audit-logs/page.tsx` now uses a shared query builder for server-side category/action/target/date filters instead of hard-coding request parameters in the page component.
- CSV export for audit logs now reuses a shared helper so row formatting stays consistent across the console and export paths.
...
- App startup wiring in `internal/app/idprovider_init.go` registers database provider and initializes identity provider system.
- All CRUD validation tests pass (12 tests); integration with app startup wiring complete.
- Verification: `go test -count=1 ./internal/idprovider ./internal/idprovider/database -v` (12 tests) and `go test ./...` (6053 tests).
...
- LDAP search filter extraction now treats negated (`NOT`) predicates as non-query constraints instead of using their child assertions for positive repository search or principal-kind narrowing.
- LDAP SearchResultEntry candidates are now post-filtered against the full LDAP filter tree, including AND/OR/NOT, equality, substring, presence, extensible matches, and AD bitwise `userAccountControl` matching rules, so repository search hints cannot over-return entries that fail the client filter.
...
- LDAPMessage decoding now rejects out-of-range `messageID` values, including zero, negative BER INTEGER encodings, values above RFC 4511 `maxInt`, and overlong integer encodings.
...
- LDAP principal entries now expose additional AD-style compatibility metadata (`canonicalName`, `instanceType`, `whenCreated`, `whenChanged`, `uSNCreated`, `uSNChanged`) and user account hints (`userAccountControl`, `accountExpires`, `primaryGroupID`) for clients that browse/categorize directory objects with Active Directory-oriented projections, and canonical-name filters narrow repository queries to the final path segment.
- LDAP extensibleMatch filters now decode RFC 4511 type/value assertions for directory attributes and `objectClass` principal-kind narrowing, improving compatibility with clients that send matching-rule-style filters.
...
- Search size-limit handling now returns `sizeLimitExceeded` only when matching entries actually exceed the client limit, preserving success for exact-limit result sets.
- LDAP directory entries now expose principal-kind-specific schema and subtrees: users as `inetOrgPerson` plus AD-compatible `user` under `ou=users`, organizations as `organizationalUnit` under `ou=organizations`, groups as `groupOfNames` under `ou=groups`, and resources as `device` under `ou=resources`.
- LDAP `groupOfNames` entries now populate requested/default `member` attributes with active Directory group-membership DNs for user, organization, group, and resource principals, while preserving a conservative fallback value for empty groups to satisfy schema MUST attributes and avoiding membership expansion for narrow non-member projections.
- LDAP principal entries now expose requested/default `memberOf` reverse membership DNs from Directory group memberships, while avoiding reverse membership expansion for narrow non-memberOf projections.
- LDAP `objectClass`/`objectCategory` filters and base DNs now narrow repository searches to the matching principal kinds, so organization-chart LDAP queries such as `objectClass=organizationalUnit` do not leak user entries and AD-style queries such as `objectCategory=person` hit the user subtree.
- LDAP repository query extraction now preserves organization/resource-oriented attributes such as `ou` and `description`, matching the gateway's accepted filter attributes.
- LDAP search results now honor base-object, one-level, and subtree scope against RFC 4514-normalized generated entry DNs for client directory browsing, including equivalent escaped separators such as `\,` and `\2c`.
...
- LDAP controls are parsed at the LDAPMessage boundary; supported critical controls (`ManageDsaIT`, Simple Paged Results, Server Side Sort, Virtual List View, Assertion, Matched Values, Domain Scope, Don't Use Copy, Subentries, LDAP Sync refreshOnly, Proxied Authorization, Dereference, Relax, No-Op, Pre/Post Read, Password Policy, Session Tracking`) are accepted, unsupported critical controls return `unavailableCriticalExtension`, Simple Paged Results uses opaque offset cookies across search pages, Server Side Sort returns the RFC response control after ordering entries, Virtual List View returns position/count windows for sorted directory browsing, RFC 4528 Assertion control can reject incompatible base-object kind assertions with `assertionFailed`, RFC 3876 Matched Values filters multi-valued response attributes, Domain Scope/Don't Use Copy are accepted for client compatibility, `subentries=true` returns an empty success result because gogomail exposes no LDAP subentries, RFC 4533 LDAP Sync refresh requests return entry sync-state controls plus a sync-done response control, Proxied Authorization only permits the already-bound authorization identity while denying other delegated identities, OpenLDAP Dereference requests are decoded and accepted as read-only no-ops until DN-valued relationship expansion exists, and common OpenLDAP general controls are accepted as read-only no-ops.
- LDAP simple bind now accepts common client bind identities including raw username/email and DN-style identities such as `uid=alice,ou=users,...`, with RFC 4514-aware first-RDN splitting, DN value unescaping, and repository authentication by user ID for generated LDAP entry DNs.
- Failed LDAP re-bind now clears the connection authorization state, and unsupported bind authentication choices return `authMethodNotSupported` instead of being treated as empty simple passwords.
...
- LDAP connection reads now accumulate valid BER PDUs beyond the initial 8 KiB read buffer up to the configured safety cap, preventing large attribute/control requests from stalling while still rejecting oversized declared messages.
- Coverage verifies Root DSE and subschema discovery, StartTLS upgrade followed by LDAP search, DN-style simple bind, unauthenticated search rejection, organization OU subtree browsing, objectClass-to-principal-kind filtering, LDAP group membership `member` DNs, LDAP reverse `memberOf` DNs, LDAP extensibleMatch parsing, LDAP search scope filtering, requested-attribute projection, AD metadata projection, AD-style user filters with bitwise `userAccountControl`, `1.1` no-attribute selection, `+` operational-attribute selection, objectClass required-attribute fallbacks, LDAP CompareRequest, Who Am I? extended operation, AbandonRequest no-response behavior, read-only write response tags, OpenLDAP `ldapsearch` plaintext, Assertion control, Matched Values control, Domain Scope/Don't Use Copy/Subentries controls, LDAP Sync refreshOnly, Proxied Authorization, Dereference, Relax/Password Policy/Session Tracking/Pre/Post Read controls, extensibleMatch, operational attributes, server-side sort, Virtual List View, StartTLS, LDAPS, paged-results interoperability, `ldapcompare`, and `ldapwhoami` interoperability, referral responses, critical-control handling, Simple Paged Results paging cookies, LDAP metrics, LDAP TLS/referral config validation, YAML/env config loading, and existing bind/search/read-only protections.
...
directory lookups. `BindRequest` (simple bind), `SearchRequest`, `CompareRequest`,
Who Am I?, StartTLS, LDAPS, referrals, Root DSE/subschema discovery, and broad
client-control compatibility are supported for authentication and directory
queries. Write operations (`Modify`, `Add`, `Delete`, `ModDN`) are rejected with
RFC 4511 response tags and `unwillingToPerform`. BER encoding/decoding handles
LDAP v3 message formats.
...
**Result**: ✅ PASS (bulk role+rate-limit behavior and fairness hardened)
- **Test setup**: bulk sender role `bulk_user`, limiter `NewBulkSenderLimiter(1, "bulk_user")` in isolation test
- **Baseline regular user latency p50**: 435.5µs
- **Under bulk load p50**: 406.25µs (-6.7% change)
- **Baseline p95**: 2.03875ms
- **Under bulk load p95**: 727.791µs (-64.3% change)
- **Target**: p95 increase ≤5% (target met in this profile)
- **Status**: PASS (bulk limiter remains effective and regular path latency is stable)
- **Gap**: none observed under this test target profile
- **Verification**: `go test -run=TestSubmissionBulkIsolation ./internal/smtp -count=1 -timeout=120s`

### Test 4: Concurrent Connections (TestSubmissionConcurrentConnections)
**Result**: ✅ PASS (no data loss and tail latency within target in latest run)
- **Concurrent sessions**: 100
- **Messages per session**: 10
- **Total expected**: 1,000 messages
- **Actually recorded**: 1,000 messages
- **Message loss rate**: 0.0% (1000/1000)
- **Latency p50**: 12.31ms
- **Latency p95**: 72.52ms
- **Latency p99**: 99.60ms (target: <100ms)
- **Status**: Data path is reliable under concurrency and tail latency target met in this run.
- **Gap**: none observed in this run; monitor again under sustained burst load.
- **Verification**: `go test -run=TestSubmissionConcurrentConnections ./internal/smtp -count=1 -timeout=120s`
...
| ≤5% isolation impact | Isolation | PASS (p95 under bulk load improved) | ✅ PASS | Bulk limiter + role binding fixed; no regression in current profile |
| 100+ concurrent sessions | Concurrency | 1000/1000 (100.0%) | ✅ PASS | No message loss and stable tail latency in latest run (p99 99.60ms) |

### Performance Gaps Identified
1. **Concurrency Latency**: Continue to monitor p99 under longer burst profiles and CI variance.
