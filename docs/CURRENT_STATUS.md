...
- LDAP `groupOfNames` entries now populate `member` with active Directory group-membership DNs for user, organization, group, and resource principals, while preserving a conservative fallback value for empty groups to satisfy schema MUST attributes.
