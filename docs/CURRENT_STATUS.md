...
- LDAP entries now provide conservative fallback values for declared objectClass MUST attributes, including `sn` for `person`/`inetOrgPerson` users and `member` for `groupOfNames` groups.
