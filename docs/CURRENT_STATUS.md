...
- LDAP principal entries now expose additional AD-style identity attributes (`distinguishedName`, deterministic `objectGUID`, `objectCategory`) for clients that key cache/display behavior on those attributes.
