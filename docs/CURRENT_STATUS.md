...
- LDAP controls are parsed at the LDAPMessage boundary; supported critical controls (`ManageDsaIT`, Simple Paged Results, Server Side Sort, Virtual List View, Assertion, Matched Values`) are accepted, unsupported critical controls return `unavailableCriticalExtension`, Simple Paged Results uses opaque offset cookies across search pages, Server Side Sort returns the RFC response control after ordering entries, Virtual List View returns position/count windows for sorted directory browsing, RFC 4528 Assertion control can reject incompatible base-object kind assertions with `assertionFailed`, and RFC 3876 Matched Values filters multi-valued response attributes.
