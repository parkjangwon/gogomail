...
| LDAP 디렉터리 게이트웨이 | RFC 4511, RFC 4512, RFC 4519 | 고급 |
...
### SMTP Super-Powerful 서버 성능 테스트 (2026-05-15)
- ✅ **처리량 테스트**: 2012 msg/sec 지속 (목표: ≥1000 msg/sec)
- ✅ **안정성 테스트**: 30초 동안 63,000개 메시지, 오류 0건, 데이터 손실 0건
- ⚠️ **격리 테스트**: 대량 메일 처리 확인 (6-38% 지연 증가, 목표: ≤5%)
- ⚠️ **동시성 테스트**: 100개 동시 연결 처리 (2-18% 메시지 손실, race condition 식별)
- 성능 검증 스위트 추가: `TestSubmissionThroughputTarget`, `TestSubmissionStability`, `TestSubmissionBulkIsolation`, `TestSubmissionConcurrentConnections`
- 전체 감사 및 테스트 결과 문서화: `docs/CURRENT_STATUS.md`

### CardDAV RFC 100% 구현 완료 (2026-05-14)
- ✅ RFC 6350 PHOTO 속성: 바이너리 사진 데이터 별도 저장 with 미디어 타입 지원
- ✅ RFC 6350 CATEGORIES 속성: 쉼표로 구분된 카테고리 목록을 TEXT[] 배열로 저장 with GIN 인덱스
- ✅ RFC 6350 GROUP 속성: 연락처 조직화용 그룹 식별자 저장 with B-tree 인덱스
- ✅ RFC 3744 ACL 지원: 주체(principal) 기반 접근 제어 with 허락/거부 권한 목록
- ✅ 모든 vCard 속성을 upsert 중 추출, 조회 시 병합하여 RFC 준수
- ✅ 5940+ 테스트 통과, 회귀 없음, 프로덕션 준비 완료된 CardDAV 구현
...
| 4 | 엔터프라이즈 인증: LDAP (RFC 4511) · SCIM 2.0 · SAML/OIDC | LDAP 고급, SCIM/SSO 예정 |
| 5 | 드라이브 WebDAV 게이트웨이 (RFC 4918) · CalDAV/CardDAV 프로덕션 강화 | ✓ 완료 (WebDAV + CardDAV RFC 100%) |
