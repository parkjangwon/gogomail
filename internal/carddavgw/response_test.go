...
func TestAddressBookHomePropertiesUsePrincipalAsCurrentUser(t *testing.T) {
	...
	if !strings.Contains(text, "<D:current-user-privilege-set><D:privilege><D:read></D:read></D:privilege><D:privilege><D:bind></D:bind></D:privilege><D:privilege><D:unbind></D:unbind></D:privilege></D:current-user-privilege-set>") {
		t.Fatalf("current-user-privilege-set should expose read, bind, and unbind privileges:\n%s", text)
	}
}
