...
func (s *fakeCardDAVDiscoveryStore) CreateAddressBookAtPath(_ context.Context, req CreateAddressBookAtPathRequest) (AddressBook, error) {
	...
	book := AddressBook{
		...
		Name:            validated.Name,
		NameLang:        validated.NameLang,
		Description:     validated.Description,
		DescriptionLang: validated.DescriptionLang,
		SyncToken:       syncToken,
		CreatedAt:       now,
		UpdatedAt:       now,
	}
...
func TestHandlerProppatchAcceptsMatchingTaggedIfHeaderPreservesLanguage(t *testing.T) {
	...
	handler := NewHandler(&store, func(*http.Request) (string, error) { return "user-1", nil })
	req := httptest.NewRequest(MethodProppatch, "/carddav/addressbooks/user-1/personal/", strings.NewReader(`<D:propertyupdate xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:carddav"><D:set><D:prop><D:displayname>Team</D:displayname><C:addressbook-description>Launch contacts</C:addressbook-description></D:prop></D:set></D:propertyupdate>`))
	req.Header.Set("If", "</carddav/addressbooks/user-1/personal/> (["+etag+"])")
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
...
func TestHandlerProppatchRejectsNonMatchingTaggedIfHeaderBeforeBodyRead(t *testing.T) {
	...
	handler := NewHandler(&store, func(*http.Request) (string, error) { return "user-1", nil })
	req := httptest.NewRequest(MethodProppatch, "/carddav/addressbooks/user-1/personal/", body)
	req.Header.Set("If", "</carddav/addressbooks/user-1/other/> (["+etag+"])")
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
...
func TestHandlerProppatchRejectsIfHeaderTrailingDataBeforeBodyRead(t *testing.T) {
	t.Parallel()
	...
	handler := NewHandler(&store, func(*http.Request) (string, error) { return "user-1", nil })
	req := httptest.NewRequest(MethodProppatch, "/carddav/addressbooks/user-1/personal/", body)
	req.Header.Set("If", `([`+etag+`]) trailing`)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	...
	if !strings.Contains(rec.Body.String(), "If header contains trailing data") {
		t.Fatalf("body = %s, want trailing data detail", rec.Body.String())
	}
	if body.reads != 0 {
		t.Fatalf("body reads = %d, want 0", body.reads)
	}
...
func TestHandlerProppatchRejectsIfHeaderMalformedPrefixBeforeBodyRead(t *testing.T) {
	t.Parallel()
	...
	handler := NewHandler(&store, func(*http.Request) (string, error) { return "user-1", nil })
	req := httptest.NewRequest(MethodProppatch, "/carddav/addressbooks/user-1/personal/", body)
	req.Header.Set("If", `garbage ([`+etag+`])`)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
...
func TestHandlerProppatchRejectsIfHeaderEmptyResourceTagBeforeBodyRead(t *testing.T) {
	t.Parallel()
...
func TestHandlerProppatchRejectsIfHeaderMalformedResourceTagSuffixBeforeBodyRead(t *testing.T) {
	t.Parallel()
	...
	handler := NewHandler(&store, func(*http.Request) (string, error) { return "user-1", nil })
	req := httptest.NewRequest(MethodProppatch, "/carddav/addressbooks/user-1/personal/", body)
	req.Header.Set("If", `</carddav/addressbooks/user-1/personal/> junk> ([`+etag+`])`)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
...
func TestHandlerProppatchRejectsIfHeaderDuplicateResourceTagDelimiterBeforeBodyRead(t *testing.T) {
	t.Parallel()
...
func TestHandlerProppatchRejectsStaleTaggedIfHeaderBeforeBodyRead(t *testing.T) {
	...
	handler := NewHandler(&store, func(*http.Request) (string, error) { return "user-1", nil })
	req := httptest.NewRequest(MethodProppatch, "/carddav/addressbooks/user-1/personal/", body)
	req.Header.Set("If", `</carddav/addressbooks/user-1/personal/> (["aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"])`)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
...
func TestHandlerProppatchAcceptsCompoundIfHeaderPreservesLanguage(t *testing.T) {
	...
	handler := NewHandler(&store, func(*http.Request) (string, error) { return "user-1", nil })
	req := httptest.NewRequest(MethodProppatch, "/carddav/addressbooks/user-1/personal/", strings.NewReader(`<D:propertyupdate xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:carddav"><D:set><D:prop><D:displayname>Team</D:displayname><C:addressbook-description>Launch contacts</C:addressbook-description></D:prop></D:set></D:propertyupdate>`))
	req.Header.Set("If", `([`+etag+`] Not <opaquelocktoken:missing-lock>)`)
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
...
func TestHandlerProppatchRejectsMalformedIfHeaderBeforeBodyRead(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if book.Name != "Personal" || book.NameLang != "ko-KR" {
				t.Fatalf("address book name mutated despite malformed If header: %+v", book)
			}
			if book.Description != "Old contacts" || book.DescriptionLang != "fr" {
				t.Fatalf("address book description mutated despite malformed If header: %+v", book)
			}
			if store.lastBookUpdate.AddressBookID != "" {
				t.Fatalf("update request recorded despite malformed If header: %+v", store.lastBookUpdate)
			}
		})
	}
}
...
func TestHandlerMkcolStoresAndPropfindReturnsAddressBookPropertyLang(t *testing.T) {
	t.Parallel()

	store := testCardDAVDiscoveryStore(t)
	handler := NewHandler(&store, func(*http.Request) (string, error) { return "user-1", nil })
	bookID := "11111111-1111-4111-8111-111111111111"
	create := httptest.NewRequest(MethodMkcol, "/carddav/addressbooks/user-1/"+bookID+"/", strings.NewReader(`<D:mkcol xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:carddav">
  <D:set>
    <D:prop xml:lang="ko-KR">
      <D:resourcetype><D:collection/><C:addressbook/></D:resourcetype>
      <D:displayname>Team Contacts</D:displayname>
	...
	create.Header.Set("Content-Type", "application/xml; charset=utf-8")
	rec := httptest.NewRecorder()
	...
	propfind := httptest.NewRequest(MethodPropfind, "/carddav/addressbooks/user-1/"+bookID+"/", strings.NewReader(`<D:propfind xmlns:D="DAV:" xmlns:C="urn:ietf:params:xml:ns:carddav">
  <D:prop>
    <D:displayname/>
    <C:addressbook-description/>
  </D:prop>
	...
	rec = httptest.NewRecorder()
	handler.ServeHTTP(rec, propfind)
...
func TestHandlerMkcolRejectsMalformedXMLLangBeforeCreate(t *testing.T) {
	t.Parallel()
  ...
  <D:set>
	...
	if _, err := store.LookupAddressBook(t.Context(), "user-1", bookID); err == nil {
		t.Fatal("address book was created despite malformed MKCOL xml:lang")
	}
}
