...
func (r BasicAuthResolver) Resolve(req *http.Request) (string, error) {
	if r.Authenticator == nil {
		return "", fmt.Errorf("caldav authenticator is required")
	}
	if req == nil {
		return "", fmt.Errorf("request is required")
	}
	if !r.AllowInsecure && !requestUsesTLS(req) {
		return "", fmt.Errorf("caldav basic authentication requires TLS")
	}
	...
	if err != nil {
		return "", fmt.Errorf("invalid caldav credentials")
	}
	if strings.TrimSpace(user.UserID) == "" || strings.ContainsAny(user.UserID, "\r\n") {
		return "", fmt.Errorf("authenticated caldav user id is invalid")
	}
