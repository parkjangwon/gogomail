...
func TestBuildPruneCalendarSyncChangesSQLUsesDirectPredicates(t *testing.T) {
	t.Parallel()

	query, args := buildPruneCalendarSyncChangesSQL(PruneCalendarSyncChangesRequest{
...
func TestBuildPruneCalendarSyncChangesSQLOmitsEmptyScopePredicates(t *testing.T) {
	t.Parallel()
	...
	if strings.Contains(query, "c.user_id =") || strings.Contains(query, "c.calendar_id =") || strings.Contains(query, "NULLIF") {
