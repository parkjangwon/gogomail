...
func TestBasicAuthResolverRejectsUnsafeRequests(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "long password", req: newBasicAuthRequest("user@example.com", strings.Repeat("p", maxCalDAVBasicPasswordBytes+1), true), want: "password is too long"},
		{name: "bad password", req: newBasicAuthRequest("user@example.com", "wrong", true), want: "invalid caldav credentials"},
	}
...
func TestBasicAuthResolverRejectsInvalidAuthenticatedUserID(t *testing.T) {
	...
	resolver := NewBasicAuthResolver(fakeCalDAVAuthenticator{username: "user@example.com", password: "secret", userID: "bad\nuser"}, true)
	req := httptest.NewRequest("PROPFIND", "/caldav/principals/user-1/", nil)
	req.SetBasicAuth("user@example.com", "secret")
...
func TestBasicAuthResolverRejectsPasswordChangeRequiredUser(t *testing.T) {
	...
	resolver := NewBasicAuthResolver(fakeCalDAVAuthenticator{
...
func (a fakeCalDAVAuthenticator) AuthenticatePlain(_ context.Context, _ string, username string, password string) (smtpd.SubmissionUser, error) {
	if username != a.username || password != a.password {
		return smtpd.SubmissionUser{}, errFakeNotFound
	}
