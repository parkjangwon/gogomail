...
func TestValidateRejectsInvalidListenerAddresses(t *testing.T) {
	tests := []struct {
		...
		{name: "imap read timeout negative", mutate: func(cfg *Config) { cfg.IMAPReadTimeout = -time.Second }},
		{name: "imap write timeout negative", mutate: func(cfg *Config) { cfg.IMAPWriteTimeout = -time.Second }},
		{name: "imap idle timeout negative", mutate: func(cfg *Config) { cfg.IMAPIdleTimeout = -time.Second }},
...
func TestValidateRejectsInvalidDeliveryDomainBackoffSettings(t *testing.T) {
	tests := []struct {
		...
		{name: "unknown scope", mutate: func(cfg *Config) { cfg.DeliveryDomainBackoffScope = "tenant_domain" }},
