...
func TestMailboxExistsEventIgnoresStaleCounts(t *testing.T) {
	t.Parallel()
	...
	for _, messages := range []uint32{2, 3} {
		err = server.writeMailboxEvent(writer, state, MailboxEvent{
		...
		if err != nil {
			t.Fatalf("writeMailboxEvent for %d messages returned error: %v", messages, err)
		}
	}
	if err := writer.Flush(); err != nil {
		t.Fatalf("flush event: %v", err)
	...
	if got := out.String(); got != "" {
		t.Fatalf("stale exists event output = %q, want empty", got)
	}
	if state.selectedMessages != 3 {
		t.Fatalf("selectedMessages = %d, want 3", state.selectedMessages)
	}
}

func TestMailboxExistsEventAppliesFreshCount(t *testing.T) {
	t.Parallel()
	...
	if got, want := out.String(), "* 5 EXISTS\r\n"; got != want {
		t.Fatalf("fresh exists event output = %q, want %q", got, want)
	}
	if state.selectedMessages != 5 {
		t.Fatalf("selectedMessages = %d, want 5", state.selectedMessages)
	}
}
...
func TestMailboxExistsEventWithoutCountIncrementsFromZero(t *testing.T) {
	t.Parallel()
	...
	if got, want := out.String(), "* 1 EXISTS\r\n"; got != want {
		t.Fatalf("initial legacy exists event output = %q, want %q", got, want)
	}
	if state.selectedMessages != 1 {
		t.Fatalf("selectedMessages = %d, want 1", state.selectedMessages)
	}
}
...
func TestMailboxExpungeEventIgnoresZeroSequence(t *testing.T) {
	t.Parallel()
	...
	if got := out.String(); got != "" {
		t.Fatalf("zero-sequence expunge output = %q, want empty", got)
	}
	if state.selectedMessages != 3 {
		t.Fatalf("selectedMessages = %d, want 3", state.selectedMessages)
	}
