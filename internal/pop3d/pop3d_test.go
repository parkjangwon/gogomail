...
func TestPOP3AuthorizationCapaIsStable(t *testing.T) {
	_, listener := newTestServer(t)
	defer listener.Close()
	...
	for _, want := range []string{"IMPLEMENTATION gogomail", "LOGIN-DELAY 0", "UIDL", "TOP", "USER", "SASL PLAIN LOGIN"} {
		if !first[want] || !second[want] {
			t.Fatalf("expected stable authorization CAPA %q in first=%#v second=%#v", want, first, second)
		}
	}
	pop3Login(t, tp)
...
func TestPOP3CommitDeletesErrorUIDLKeepsDeleteMarkClear(t *testing.T) {
	mb := &commitMailbox{
		mockMailbox: &mockMailbox{
			messages: []mockMessage{
				{uidl: "msg001", size: 42, content: "Hello\r\n"},
	...
	if mb.Deleted(0) {
		t.Fatal("expected UIDL after failed QUIT to leave delete mark clear")
	}
	...
	if mb.commitCalls != 1 {
		t.Fatalf("CommitDeletes calls after UIDL then no-delete QUIT = %d, want 1", mb.commitCalls)
	}
}

func TestPOP3CommitDeletesErrorLISTKeepsDeleteMarkClear(t *testing.T) {
	mb := &commitMailbox{
		mockMailbox: &mockMailbox{
			messages: []mockMessage{
				{uidl: "msg001", size: 42, content: "Hello\r\n"},
	...
	if mb.Deleted(0) {
		t.Fatal("expected LIST after failed QUIT to leave delete mark clear")
	}
	...
	if mb.commitCalls != 1 {
		t.Fatalf("CommitDeletes calls after LIST then no-delete QUIT = %d, want 1", mb.commitCalls)
	}
}
...
func TestPOP3CommitDeletesErrorAllowsRset(t *testing.T) {
	mb := &commitMailbox{
		mockMailbox: &mockMailbox{
			messages: []mockMessage{
				{uidl: "msg001", size: 42, content: "Hello\r\n"},
	...
	if line := pop3Cmd(t, tp, "+OK", "LIST 1"); !strings.Contains(line, "1 42") {
		t.Fatalf("expected LIST 1 after RSET following failed QUIT to restore message size, got: %s", line)
	}
	if mb.Deleted(0) {
		t.Fatal("expected RSET after failed QUIT to leave delete mark clear")
	}
}

func TestPOP3CommitDeletesErrorRejectsInvalidDele(t *testing.T) {
	mb := &commitMailbox{
		mockMailbox: &mockMailbox{
			messages: []mockMessage{
				{uidl: "msg001", size: 42, content: "Hello\r\n"},
	...
	pop3Cmd(t, tp, "+OK", "DELE 1")
	pop3Cmd(t, tp, "-ERR", "QUIT")
	pop3Cmd(t, tp, "-ERR", "DELE 2")

	if line := pop3Cmd(t, tp, "+OK", "LIST 1"); !strings.Contains(line, "1 42") {
		t.Fatalf("expected LIST 1 after invalid DELE following failed QUIT to restore message size, got: %s", line)
	}
	if mb.Deleted(0) {
		t.Fatal("expected invalid DELE after failed QUIT to leave delete mark clear")
	}
}
...
func TestPOP3QuitAfterFailedCommitAllowsRedeletedRetry(t *testing.T) {
	...
	if mb.Deleted(0) {
		t.Fatal("expected failed QUIT to roll back delete mark before retry")
	}
	...
	if mb.commitCalls != 2 {
		t.Fatalf("CommitDeletes calls after re-delete retry = %d, want 2", mb.commitCalls)
	}
	if !mb.Deleted(0) {
		t.Fatal("expected successful re-delete retry to preserve committed delete mark")
	}
}
...
func TestPOP3QuitWithoutDeletesClosesConnection(t *testing.T) {
	mb := &commitMailbox{
		mockMailbox: &mockMailbox{
			messages: []mockMessage{
				{uidl: "msg001", size: 42, content: "Hello\r\n"},
	...
	if line, err := tp.ReadLine(); err == nil {
		t.Fatalf("expected connection close after no-delete QUIT, got line: %s", line)
	}
}
