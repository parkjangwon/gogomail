...
func TestSessionResetsDSNAndDomainPolicyAfterFailedData(t *testing.T) {
	t.Parallel()
	...
	raw := "Message-ID: <dsn-policy-failed@example.org>\r\nSubject: too large for d2\r\n\r\nbody"
	requireSMTPStatus(t, session.Data(strings.NewReader(raw)), 552, gosmtp.EnhancedCode{5, 3, 4})
	...
	if err := session.Data(strings.NewReader(raw)); err != nil {
		t.Fatalf("d1-only Data after failed DATA returned error: %v", err)
	}
	if len(recorder.messages) != 1 {
		t.Fatalf("recorded messages = %d, want 1", len(recorder.messages))
	}
	...
	if got.Return != "" || got.EnvelopeID != "" {
		t.Fatalf("recorded DSN envelope = %+v, want no failed transaction DSN leak", got)
	}
	if len(got.Recipients) != 1 || len(got.Recipients[0].Notify) != 0 || got.Recipients[0].OriginalRecipient != "" {
		t.Fatalf("recorded DSN recipients = %+v, want accepted address without failed transaction options", got.Recipients)
	}
}
...
func TestSessionResetsDSNAndDomainPolicyAfterRSET(t *testing.T) {
	t.Parallel()
	...
	raw := "Message-ID: <rset-dsn-reset@example.org>\r\nSubject: larger than d2\r\n\r\nbody"
	if err := session.Data(strings.NewReader(raw)); err != nil {
		t.Fatalf("d1-only Data after RSET returned error: %v", err)
	}
	if len(recorder.messages) != 1 {
	...
	got := recorder.messages[0].DSN
	if got.Return != "" || got.EnvelopeID != "" {
		t.Fatalf("recorded DSN envelope = %+v, want no reset transaction DSN leak", got)
	}
	if len(got.Recipients) != 1 || len(got.Recipients[0].Notify) != 0 || got.Recipients[0].OriginalRecipient != "" {
		t.Fatalf("recorded DSN recipients = %+v, want accepted address without reset transaction options", got.Recipients)
	}
}

func TestSessionResetsMixedDomainPolicyOnNewMAIL(t *testing.T) {
	t.Parallel()
	...
	raw := "Message-ID: <mail-reset-policy@example.org>\r\nSubject: larger than d2\r\n\r\nbody"
	if err := session.Data(strings.NewReader(raw)); err != nil {
		t.Fatalf("d1-only Data after new MAIL returned error: %v", err)
	}
	if len(recorder.messages) != 1 {
		t.Fatalf("recorded messages = %d, want 1", len(recorder.messages))
	}
...
func TestSessionResetsDSNAndDomainPolicyOnNewMAIL(t *testing.T) {
	t.Parallel()
	...
	raw := "Message-ID: <mail-dsn-reset@example.org>\r\nSubject: larger than d2\r\n\r\nbody"
	if err := session.Data(strings.NewReader(raw)); err != nil {
		t.Fatalf("d1-only Data after new MAIL returned error: %v", err)
	}
	if len(recorder.messages) != 1 {
	...
	got := recorder.messages[0].DSN
	if got.Return != "" || got.EnvelopeID != "" {
		t.Fatalf("recorded DSN envelope = %+v, want no previous MAIL DSN leak", got)
	}
	if len(got.Recipients) != 1 || len(got.Recipients[0].Notify) != 0 || got.Recipients[0].OriginalRecipient != "" {
		t.Fatalf("recorded DSN recipients = %+v, want accepted address without previous MAIL options", got.Recipients)
	}
}
