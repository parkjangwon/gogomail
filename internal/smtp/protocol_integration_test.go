...
func TestSMTPProtocolTrustedRelayRejectsUntrustedClient(t *testing.T) {
	...
	trusted, err := NewStaticTrustedRelays([]string{"192.0.2.0/24"})
	if err != nil {
	...
	code, msg, err := rawProtocolCommandCode(text, "MAIL FROM:<sender@example.net>")
	if err != nil {
		t.Fatalf("MAIL FROM returned transport error: %v", err)
	}
	if code != 550 || !strings.Contains(msg, "not trusted") {
		t.Fatalf("MAIL FROM code/message = %d %q, want trusted relay policy rejection", code, msg)
	}
	if err := rawProtocolCommand(text, 221, "QUIT"); err != nil {
		t.Fatalf("QUIT returned error: %v", err)
	}
...
func TestSMTPProtocolEHLOResetsMixedDomainPolicy(t *testing.T) {
	t.Parallel()
	...
	if err := rawProtocolCommand(text, 250, "EHLO client.example.net"); err != nil {
		t.Fatalf("first EHLO returned error: %v", err)
	}
	...
	if err := rawProtocolCommand(text, 250, "RCPT TO:<two@example.net>"); err != nil {
		t.Fatalf("second RCPT returned error: %v", err)
	}
	if err := rawProtocolCommand(text, 250, "EHLO client.example.net"); err != nil {
		t.Fatalf("second EHLO returned error: %v", err)
	}
	if err := rawProtocolCommand(text, 250, "MAIL FROM:<sender@example.org>"); err != nil {
	...
	writeProtocolData(t, text, "Message-ID: <ehlo-reset@example.org>\r\nSubject: larger than d2\r\n\r\nbody\r\n")
	if err := rawProtocolCommand(text, 221, "QUIT"); err != nil {
		t.Fatalf("QUIT returned error: %v", err)
	}
...
func TestSMTPProtocolEHLOResetsDSNAndMixedDomainPolicy(t *testing.T) {
	t.Parallel()
	...
	if err := rawProtocolCommand(text, 250, "MAIL FROM:<sender@example.org> RET=HDRS ENVID=ehlo-env"); err != nil {
		t.Fatalf("MAIL returned error: %v", err)
	}
	...
	writeProtocolData(t, text, "Message-ID: <ehlo-dsn-reset@example.org>\r\nSubject: larger than d2\r\n\r\nbody\r\n")
	if err := rawProtocolCommand(text, 221, "QUIT"); err != nil {
		t.Fatalf("QUIT returned error: %v", err)
	}
	...
	if got.Return != "" || got.EnvelopeID != "" {
		t.Fatalf("recorded DSN envelope = %+v, want no pre-EHLO DSN leak", got)
	}
	if len(got.Recipients) != 1 || len(got.Recipients[0].Notify) != 0 || got.Recipients[0].OriginalRecipient != "" {
		t.Fatalf("recorded DSN recipients = %+v, want accepted address without pre-EHLO options", got.Recipients)
	}
}

func TestSMTPProtocolHELOResetsMixedDomainPolicy(t *testing.T) {
	t.Parallel()
	...
	writeProtocolData(t, text, "Message-ID: <helo-reset@example.org>\r\nSubject: larger than d2\r\n\r\nbody\r\n")
	if err := rawProtocolCommand(text, 221, "QUIT"); err != nil {
		t.Fatalf("QUIT returned error: %v", err)
	}
...
func TestSMTPProtocolQUITIsolatesMixedDomainPolicy(t *testing.T) {
	...
	first := textproto.NewConn(firstConn)
	if _, _, err := first.ReadResponse(220); err != nil {
		t.Fatalf("first greeting returned error: %v", err)
	}
	if err := rawProtocolCommand(first, 250, "EHLO client.example.net"); err != nil {
		t.Fatalf("first EHLO returned error: %v", err)
	}
	if err := rawProtocolCommand(first, 250, "MAIL FROM:<sender@example.org>"); err != nil {
		t.Fatalf("first MAIL returned error: %v", err)
	}
	if err := rawProtocolCommand(first, 250, "RCPT TO:<one@example.com>"); err != nil {
		t.Fatalf("first RCPT returned error: %v", err)
	}
	if err := rawProtocolCommand(first, 250, "RCPT TO:<two@example.net>"); err != nil {
		t.Fatalf("second RCPT returned error: %v", err)
	}
	if err := rawProtocolCommand(first, 221, "QUIT"); err != nil {
	...
	defer second.Close()
	if _, _, err := second.ReadResponse(220); err != nil {
		t.Fatalf("second greeting returned error: %v", err)
	}
	if err := rawProtocolCommand(second, 250, "EHLO client.example.net"); err != nil {
		t.Fatalf("second EHLO returned error: %v", err)
	}
	if err := rawProtocolCommand(second, 250, "MAIL FROM:<sender@example.org>"); err != nil {
		t.Fatalf("second MAIL returned error: %v", err)
	}
	if err := rawProtocolCommand(second, 250, "RCPT TO:<one@example.com>"); err != nil {
		t.Fatalf("third RCPT returned error: %v", err)
	}
	writeProtocolData(t, second, "Message-ID: <quit-isolation@example.org>\r\nSubject: larger than d2\r\n\r\nbody\r\n")
	if err := rawProtocolCommand(second, 221, "QUIT"); err != nil {
...
func TestSMTPProtocolQUITIsolatesDSNAndMixedDomainPolicy(t *testing.T) {
	t.Parallel()
	...
	if err := rawProtocolCommand(first, 250, "MAIL FROM:<sender@example.org> RET=HDRS ENVID=quit-env"); err != nil {
		t.Fatalf("first MAIL returned error: %v", err)
	}
	if err := rawProtocolCommand(first, 250, "RCPT TO:<one@example.com> NOTIFY=FAILURE ORCPT=rfc822;one@example.com"); err != nil {
		t.Fatalf("first RCPT returned error: %v", err)
	}
	if err := rawProtocolCommand(first, 250, "RCPT TO:<two@example.net> NOTIFY=DELAY ORCPT=rfc822;two@example.net"); err != nil {
		t.Fatalf("second RCPT returned error: %v", err)
	}
	if err := rawProtocolCommand(first, 221, "QUIT"); err != nil {
		t.Fatalf("first QUIT returned error: %v", err)
	...
	writeProtocolData(t, second, "Message-ID: <quit-dsn-isolation@example.org>\r\nSubject: larger than d2\r\n\r\nbody\r\n")
	if err := rawProtocolCommand(second, 221, "QUIT"); err != nil {
		t.Fatalf("second QUIT returned error: %v", err)
	}
	...
	got := recorder.messages[0].DSN
	if got.Return != "" || got.EnvelopeID != "" {
		t.Fatalf("recorded DSN envelope = %+v, want no previous connection DSN leak", got)
	}
	if len(got.Recipients) != 1 || len(got.Recipients[0].Notify) != 0 || got.Recipients[0].OriginalRecipient != "" {
		t.Fatalf("recorded DSN recipients = %+v, want accepted address without previous connection options", got.Recipients)
	}
}
