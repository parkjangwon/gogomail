...
func TestWebmailCapabilitiesHandler(t *testing.T) {
	...
	if !got.Drive.UploadSessions || !got.Drive.ListUploadSessions || !got.Drive.NodeNameSearch || !got.Drive.NodeAllParentsSearch || !got.Drive.NodeTypeFilter || len(got.Drive.SupportedNodeTypes) != 2 || got.Drive.SupportedNodeTypes[0] != drive.NodeTypeFolder || !got.Drive.NodeSortControls || len(got.Drive.SupportedNodeSorts) != 4 || got.Drive.SupportedNodeSorts[0] != drive.NodeSortName || !got.Drive.NodeDownload || !got.Drive.NodeRangeDownload || !got.Drive.CopyNodes || !got.Drive.ShareLinks || len(got.Drive.ShareLinkPermissions) != 2 || got.Drive.MaxShareLinkTTLSeconds != int64(drive.MaxShareLinkTTL.Seconds()) || !got.Drive.UsageSummary || !got.Drive.FinalizeUploadSessions || !got.Drive.ResumableChunkedUploads || got.Drive.MaxUploadSessionBytes != drive.MaxUploadSessionBytes {
...
func TestMailRoutesAllowAPIKeyReadForDomainUser(t *testing.T) {
	...
	service := &fakeMessageService{
		userProfile: maildb.UserProfile{UserID: "user-1", DomainID: "domain-1"},
		list:        []maildb.MessageSummary{{ID: "msg-1", Subject: "hello"}},
	}
	mux := http.NewServeMux()
	RegisterMailRoutes(mux, service, manager)
	...
	if service.lastSend.UserID != "" {
		t.Fatalf("send should not be called: %+v", service.lastSend)
	}
	if service.lastUserID != "user-1" {
...
func TestMailRoutesAllowAPIKeyUserHeader(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/mailbox/overview", nil)
...
func TestMailRoutesRejectAPIKeyCrossDomainUser(t *testing.T) {
	t.Parallel()

	service := &fakeMessageService{
		userProfile: maildb.UserProfile{UserID: "user-1", DomainID: "domain-2"},
	}
	mux := http.NewServeMux()
	RegisterMailRoutes(mux, service, nil)
...
func TestMailRoutesAllowAPIKeySendWithBodyUser(t *testing.T) {
	t.Parallel()
