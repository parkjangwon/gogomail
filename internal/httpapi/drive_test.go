...
func TestDriveListNodesHandler(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/drive/nodes?user_id=user-1&parent_id=parent-1&status=active&q=%20Report%20&sort=updated&limit=25", nil)
	...
	if service.listReq.UserID != "user-1" || service.listReq.ParentID != "parent-1" || service.listReq.Status != "active" || service.listReq.Query != "Report" || service.listReq.Sort != drive.NodeSortUpdated || service.listReq.Limit != 25 {
