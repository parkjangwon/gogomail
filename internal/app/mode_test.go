...
func TestParseModeAcceptsKnownBackendModes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "drive cleanup worker", raw: "drive-cleanup-worker", want: ModeDriveCleanup},
		{name: "dav sync retention worker", raw: "dav-sync-retention-worker", want: ModeDAVSyncRetention},
		...
		{name: "pop3", raw: "pop3", want: ModePOP3},
		{name: "caldav", raw: "caldav", want: ModeCalDAV},
		{name: "carddav", raw: "carddav", want: ModeCardDAV},
		{name: "webdav", raw: "webdav", want: ModeWebDAV},
