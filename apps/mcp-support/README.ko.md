...
툴 이름은 `{공급자}_{동작}_{대상}` 패턴을 따릅니다. GoGoMail **액션** 툴(쓰기 작업)은 모두 감사 로그가 기록됩니다 — `ticketId`가 지정되면 해당 Suppo 티켓에 내부 댓글로 기록되고, 생략하면 자동으로 별도 감사 티켓이 생성됩니다. Suppo가 설정되지 않은 경우 감사 기록은 stderr에 출력됩니다.
...
| `gogomail_delete_dlq_entry` | `DELETE /admin/v1/dlq/{id}?stream=` | 막힌 DLQ 항목 삭제. *(감사 로그 기록)* |
...
GoGoMail **액션** 툴(쓰기 작업)은 모두 실행 후 자동으로 감사 메모를 기록합니다. 메모에는 다음이 포함됩니다:
