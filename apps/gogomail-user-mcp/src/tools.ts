...
export const toolDefinitions: Tool[] = [
  ...
  { name: "gogomail_mail_send", description: "Send mail using POST /api/v1/messages/send. In basic permission mode confirm must equal `send message`.", inputSchema: { type: "object", properties: { to: { type: "array", items: { type: "object", properties: { email: { type: "string", format: "email" }, name: { type: "string" } }, required: ["email"] } }, cc: { type: "array" }, bcc: { type: "array" }, subject: { type: "string" }, text_body: { type: "string" }, html_body: { type: "string" }, intent: { type: "string", enum: ["new", "reply", "forward"] }, source_message_id: { type: "string" }, attachment_ids: { type: "array", items: { type: "string" }, maxItems: 100 }, confirm: { type: "string" } }, required: ["to", "subject"] } },
  ...
  { name: "gogomail_drive_copy", description: "Copy a Drive node using POST /api/v1/drive/nodes/{id}/copy.", inputSchema: { type: "object", properties: { id: { type: "string", maxLength: 200 }, parent_id: { type: "string", maxLength: 200 }, name: { type: "string" } }, required: ["id"] } },
...
const schemas: Record<string, z.ZodTypeAny> = {
  ...
  gogomail_mail_send: z.object({ to: z.array(address).min(1).max(200), cc: z.array(address).max(200).optional(), bcc: z.array(address).max(200).optional(), subject: z.string().max(998), text_body: z.string().optional(), html_body: z.string().optional(), intent: z.enum(["new", "reply", "forward"]).default("new"), source_message_id: optionalID, attachment_ids: z.array(id).max(100).optional(), confirm }),
  ...
  gogomail_drive_copy: z.object({ id, parent_id: optionalID, name: z.string().max(255).optional() }),
...
export async function callTool(client: GogomailUserClient, name: string, rawArgs: Record<string, unknown>, envMode: "basic" | "bypass"): Promise<unknown> {
  ...
  const requireConfirm = (expected: string) => {
  ...
  switch (name) {
    ...
    case "gogomail_mail_send": {
      requireConfirm("send message");
      const body = withMCPNotice({ ...args } as { text_body?: string; html_body?: string }, settings);
      return client.request("POST", "/api/v1/messages/send", { ...args, ...body, confirm: undefined });
    }
    ...
    case "gogomail_mail_send_draft":
      ...
      return client.request("POST", `/api/v1/drafts/${encodeURIComponent(String(args.id))}/send`);
    case "gogomail_mail_delete_draft":
      requireConfirm(`delete draft ${args.id}`);
      return client.request("DELETE", `/api/v1/drafts/${encodeURIComponent(String(args.id))}`);
    ...
    case "gogomail_mail_delete_message":
      ...
      return client.request("DELETE", `/api/v1/messages/${encodeURIComponent(String(args.id))}`);
    ...
    case "gogomail_contacts_delete_addressbook":
      ...
      return client.request("DELETE", `/api/mail/addressbooks/${encodeURIComponent(String(args.addressbook_id))}`);
    ...
    case "gogomail_contacts_delete":
      ...
      return client.request("DELETE", `/api/mail/addressbooks/${encodeURIComponent(String(args.addressbook_id))}/contacts/${encodeURIComponent(String(args.object_name))}`);
    ...
    case "gogomail_drive_trash":
      ...
      return client.request("POST", `/api/v1/drive/nodes/${encodeURIComponent(String(args.id))}/trash`);
    ...
    case "gogomail_drive_delete":
      ...
      return client.request("DELETE", `/api/v1/drive/nodes/${encodeURIComponent(String(args.id))}`);
    case "gogomail_drive_share_link":
      requireConfirm(`share drive ${args.id}`);
      return client.request("POST", `/api/v1/drive/nodes/${encodeURIComponent(String(args.id))}/share-links`, { permission: args.permission, expires_at: args.expires_at, password: args.password });
    ...
    case "gogomail_calendar_delete":
      ...
      return client.request("DELETE", `/api/v1/calendars/${encodeURIComponent(String(args.calendar_id))}`);
    ...
    case "gogomail_calendar_delete_object":
      ...
      return client.request("DELETE", `/api/v1/calendars/${encodeURIComponent(String(args.calendar_id))}/objects/${encodeURIComponent(String(args.object_name))}`);
  }
}
