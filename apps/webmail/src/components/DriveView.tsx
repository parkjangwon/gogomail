...
import {
  ...
  renameDriveNode, trashDriveNode, downloadDriveNode,
  uploadDriveFile, createDriveShareLink, listDriveShareLinks, revokeDriveShareLink,
} from '@/lib/api';
import {
  FolderIcon, DocumentIcon, ArrowUpTrayIcon, FolderPlusIcon,
  EllipsisVerticalIcon, ArrowDownTrayIcon, LinkIcon, PencilIcon,
  TrashIcon, XMarkIcon, ArrowPathIcon, ChevronRightIcon,
...
export function DriveView() {
  ...
  return (
    <div
      style={{ flex: 1, minWidth: 0, height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--color-bg-primary)', position: 'relative' }}
    ...
    >
      ...
      {/* Usage bar */}
      {usage && (
        <div style={{ padding: '6px 20px', borderBottom: '1px solid var(--color-border-subtle)', display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
          <div style={{ flex: 1, height: '4px', borderRadius: '2px', background: 'var(--color-bg-tertiary)', overflow: 'hidden' }}>
            <div style={{ height: '100%', borderRadius: '2px', width: `${usedPct}%`, background: usedPct > 80 ? 'var(--color-destructive)' : 'var(--color-accent)', transition: 'width 400ms ease' }} />
          </div>
          <span style={{ fontSize: '11px', color: 'var(--color-text-tertiary)', whiteSpace: 'nowrap' }}>
            {formatBytes(usage.quota_used)} / {formatBytes(usage.quota_limit)}
          </span>
        </div>
      )}
