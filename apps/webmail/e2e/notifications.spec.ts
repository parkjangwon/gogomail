...
test.describe('Notification center', () => {
  ...
  test('prunes unknown stored notification fields after initial page hydration', async ({ page }) => {
    await page.evaluate(() => {
      const raw = JSON.stringify([
        {
          id: 'initial-extra-field-storage',
          category: 'system',
          severity: 'info',
          title: 'Initial sanitized stored notification',
          body: 'Known body',
          actionUrl: '/mail?from=initial-storage',
          timestamp: Date.now(),
          read: false,
          admin: true,
          nested: { leak: true },
    ...
    await expect.poll(
      () => page.evaluate(() => {
        ...
        const notification = raw ? JSON.parse(raw)[0] as Record<string, unknown> | undefined : undefined;
        return notification ? Object.keys(notification).sort() : [];
      }),
      { timeout: 5_000 },
    ).toEqual(['actionUrl', 'body', 'category', 'id', 'read', 'severity', 'timestamp', 'title']);
  ...
  test('clear-all returns to all notifications when filtered clear leaves hidden items', async ({ page }) => {
    await pushNotification(page, { title: 'Deployment finished', body: 'System job succeeded', category: 'system' });
    await pushNotification(page, { title: 'Inbox delivery', body: 'Mail from Finance', category: 'mail_received' });
