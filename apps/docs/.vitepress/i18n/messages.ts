...
const copy = {
  en: {
    ...
    webmailShellBody: 'The webmail app opens on mail and provides adjacent app surfaces for DM, calendar, contacts, drive, and settings. The guide should describe those as visible surfaces, not as hidden backend capabilities.',
  ...
  ko: {
    ...
    webmailShellBody: '웹메일은 메일 화면에서 시작하고 DM, 캘린더, 연락처, 드라이브, 설정으로 이동할 수 있는 앱 표면을 제공합니다. 이 가이드는 이를 숨은 백엔드 기능이 아니라 화면에 보이는 표면으로 설명합니다.',
  ...
  ja: {
    ...
    webmailShellBody: 'ウェブメールはメール画面から始まり、DM、カレンダー、連絡先、ドライブ、設定へ移動できる面を提供します。隠れたバックエンド機能ではなく、表示される面として説明します。',
  ...
  'zh-CN': {
    ...
    webmailShellBody: 'Webmail 从邮件页面开始，并提供可进入私信、日历、联系人、Drive 和设置的应用区域。指南应把它们描述为可见界面，而不是隐藏后端能力。',
...
const featureCopy = {
  en: {
    ...
    webmailAppsTitle: 'DM, calendar, contacts, and drive',
  ...
  ko: {
    ...
    webmailAppsTitle: 'DM, 캘린더, 연락처, 드라이브',
  ...
  ja: {
    ...
    webmailAppsTitle: 'DM、カレンダー、連絡先、ドライブ',
  ...
  'zh-CN': {
    ...
    webmailAppsTitle: 'DM、日历、联系人和 Drive',
  ...
  "error": "api key scope mail:read is required"
      ...
      adminGovernance: {
        ...
        sections: [
          ...
          {
            title: console.nav.spam_filter,
            body: translated({
              en: 'Spam filtering is managed from the company security area and can be scoped to a specific domain override. Administrators tune thresholds, quarantine versus reject behavior, RBL zones, attachment controls, sender allow/block lists, built-in filter-pack toggles, and tenant-owned custom phrase packs, then review recent filter events and counters from mail-flow evidence.',
              ko: '스팸 필터는 회사 보안 영역에서 관리하며, 특정 도메인 오버라이드로 범위를 좁힐 수 있습니다. 관리자는 임계값, 격리 또는 거부 동작, RBL 영역, 첨부파일 제어, 발신자 허용/차단 목록, 내장 필터팩 토글, 테넌트 소유 커스텀 문구 팩을 조정하고 메일 흐름 근거에서 최근 필터 이벤트와 카운터를 확인합니다.',
              ja: 'スパムフィルターは会社のセキュリティ領域で管理し、特定ドメインのオーバーライドに絞ることができます。管理者はしきい値、隔離または拒否、RBL ゾーン、添付制御、送信者の許可/ブロックリスト、組み込みフィルターパックの切り替え、テナント所有のカスタム語句パックを調整し、メールフローの証跡から最近のフィルターイベントとカウンターを確認します。',
              'zh-CN': '垃圾邮件过滤在公司安全区域管理，也可以限定到特定域名覆盖策略。管理员可调整阈值、隔离或拒收动作、RBL 区域、附件控制、发件人允许/阻止列表、内置过滤包开关和租户自有自定义短语包，并从邮件流证据查看最近的过滤事件和计数器。',
            }),
            items: ['filter_packs', 'rbl_zones', console.nav.smtp_policy, console.nav.suppression_list, console.nav.mail_flow_logs],
          },
      ...
      webmailApps: {
        ...
        sections: [
          ...
          {
            title: webmail.nav.dm,
            body: translated({
              en: 'DM opens participant-only direct and group rooms. Each room stores messages with per-room AES-256-GCM encryption. Direct rooms are 1:1; group rooms can be public or private with owner-controlled membership, invites, and media/link views.',
              ko: 'DM은 참여자 전용 1:1 및 그룹 룸을 제공합니다. 각 룸은 룸별 AES-256-GCM 암호화로 메시지를 저장합니다. 1:1 룸은 두 사람 사이, 그룹 룸은 공개 또는 비공개로 소유자가 초대와 멤버십을 관리하며 미디어/링크 뷰를 지원합니다.',
              ja: 'DM は参加者専用の 1:1 およびグループルームを提供します。各ルームは AES-256-GCM 暗号化でメッセージを保存します。1:1 ルームは 2 人の間、グループルームは公開または非公開で所有者が招待とメンバーを管理し、メディア/リンクビューに対応します。',
              'zh-CN': 'DM 提供参与者专属的 1:1 及群组房间。每个房间使用 AES-256-GCM 逐房间加密存储消息。1:1 为两人对话，群组可公开或私密，所有者管理邀请和成员，支持媒体/链接视图。',
            }),
            items: [webmail.nav.dm, webmail.dmPanel.newChat, webmail.dmPanel.directMessage, webmail.dmPanel.group],
          },
        ...
        sections: [
          {
            ...
            body: translated({
              en: 'Global shortcuts work from anywhere in the mailbox. Use ? for the shortcut guide, Cmd+K or Ctrl+K for Spotlight, [ to toggle the sidebar, b for Notification Center, s to compose new mail, and ` to open DM.',
              ko: '전역 단축어는 메일함 어디서나 동작합니다. ?는 단축키 도움말, Cmd+K 또는 Ctrl+K는 스팟라이트, [는 사이드바 토글, b는 알림 센터, s는 새 메일 작성, `는 DM 열기입니다.',
              ja: 'グローバルショートカットはメールボックス全体で動作します。? でショートカットガイド、Cmd+K または Ctrl+K でスポットライト、[ でサイドバー切替、b で通知センター、s で新規メール作成、` で DM を開きます。',
              'zh-CN': '全局快捷键在邮箱任意位置生效。? 打开快捷键指南，Cmd+K 或 Ctrl+K 打开 Spotlight，[ 切换侧边栏，b 打开通知中心，s 写新邮件，` 打开私信。',
            }),
            items: ['?', 'Cmd+K / Ctrl+K', '[', 'b', 's', '`'],
          },
          {
            title: translated({ en: 'App switch and Spotlight', ko: '앱 전환과 스팟라이트', ja: 'アプリ切替とスポットライト', 'zh-CN': '应用切换与 Spotlight' }),
            body: translated({
              en: 'Use app-switch shortcuts to move between mail, calendar, contacts, drive, and settings. Spotlight search covers mail, contacts, calendar, and drive results from one search surface.',
              ko: '앱 전환 단축어는 메일, 캘린더, 연락처, 드라이브, 설정 사이를 이동할 때 사용합니다. 스팟라이트 검색은 하나의 검색창에서 메일, 연락처, 일정, 드라이브 결과를 함께 찾습니다.',
              ja: 'アプリ切替ショートカットは、メール、カレンダー、連絡先、ドライブ、設定の移動に使います。スポットライト検索は、1 つの検索面でメール、連絡先、予定表、ドライブを横断します。',
              'zh-CN': '应用切换快捷键用于在邮件、日历、联系人、Drive 和设置之间移动。Spotlight 搜索在一个搜索界面中覆盖邮件、联系人、日历和 Drive 结果。',
            }),
            items: ['g m', 'g c', 'g a', 'g d', 'g ,'],
          },
          {
            title: translated({ en: 'Mail navigation and reading', ko: '메일 이동과 읽기', ja: 'メール移動と閲覧', 'zh-CN': '邮件导航与阅读' }),
            body: translated({
              en: 'When a message row is focused, o opens the reading view. j / k or ↑ / ↓ move through the message list, Space toggles the checkbox, and Home / End jump to first or last. Esc closes the reading view and returns to the list.',
              ko: '메시지 행에 포커스가 있을 때 o를 누르면 읽기 화면이 열립니다. j / k 또는 ↑ / ↓로 목록을 이동하고, Space로 체크박스를 토글하며, Home / End로 처음/끝으로 이동합니다. Esc는 읽기 화면을 닫고 목록으로 돌아갑니다.',
              ja: 'メッセージ行にフォーカスがあるとき、o で閲覧画面を開きます。j / k または ↑ / ↓ で一覧を移動し、Space でチェックボックスをトグル、Home / End で先頭/末尾へ移動します。Esc で閲覧画面を閉じて一覧に戻ります。',
              'zh-CN': '邮件行获得焦点时，按 o 打开阅读视图。j / k 或 ↑ / ↓ 在列表中移动，Space 切换复选框，Home / End 跳到首尾。Esc 关闭阅读视图并返回列表。',
            }),
            items: ['j / k', '↑ / ↓', 'o', 'Space', 'Home / End', 'Ctrl+A', 'Esc'],
          },
          { title: translated({ en: 'Mail actions', ko: '메일 작업', ja: 'メール操作', 'zh-CN': '邮件操作' }), body: f.sourceNote, items: ['r', 'a', 'f', 'e', 'v', '#', 'm', 'Shift+M', 'z', 'l', '!'] },
          { title: webmail.mail.folders, body: f.sourceNote, items: ['g i', 'g s', 'g t', 'g p'] },
          { title: webmail.compose.newMessage, body: f.sourceNote, items: ['Ctrl+Enter', 'Ctrl+S', 'Esc'] },
          {
            title: translated({ en: 'Calendar', ko: '캘린더', ja: 'カレンダー', 'zh-CN': '日历' }),
            body: f.sourceNote,
            items: ['d', 'w', 'm', 't', '← / →'],
          },
          {
