      ...
      adminGovernance: {
        ...
        sections: [
          ...
          { title: console.nav.dmarc_spf, body: screenBody(console.nav.dmarc_spf), items: [console.nav.domains, console.nav.dkim_keys] },
          {
            ...
            body: translated({
              en: 'Spam filtering is managed from the company security area and can be scoped to a specific domain override. Administrators tune the score threshold, quarantine versus reject behavior, blocked attachment extensions, sender allow/block lists, and then review recent filter events and counters from mail-flow evidence.',
              ko: '스팸 필터는 회사 보안 영역에서 관리하며, 특정 도메인 오버라이드로 범위를 좁힐 수 있습니다. 관리자는 점수 임계값, 격리 또는 거부 동작, 차단 첨부 확장자, 발신자 허용/차단 목록을 조정하고 메일 흐름 근거에서 최근 필터 이벤트와 카운터를 확인합니다.',
              ja: 'スパムフィルターは会社のセキュリティ領域で管理し、特定ドメインのオーバーライドに絞ることができます。管理者はスコアしきい値、隔離または拒否、ブロックする添付拡張子、送信者の許可/ブロックリストを調整し、メールフローの証跡から最近のフィルターイベントとカウンターを確認します。',
              'zh-CN': '垃圾邮件过滤在公司安全区域管理，也可以限定到特定域名覆盖策略。管理员可调整评分阈值、隔离或拒收动作、阻止的附件扩展名、发件人允许/阻止列表，并从邮件流证据查看最近的过滤事件和计数器。',
            }),
            items: [console.nav.smtp_policy, console.nav.suppression_list, console.nav.mail_flow_logs],
          },
